...
* Callback returns a object with all sections structured
...
*/
getSections = function(directory, options, callback) {
	traverse(directory, options, function(err, styleguide) {
		var data = [];

		_.forEach(styleguide.sections, function(section){
			if(section.refParts.length == 1){
				data[section.refParts[0]] = section;
				data[section.refParts[0]].sections = [];
			}
		});

		_.forEach(styleguide.sections, function(section){
			if(section.refParts.length == 2){
				if(_.isUndefined(data[section.refParts[0]])){
					data[section.refParts[0]] = { sections: [] };
				}
				data[section.refParts[0]].sections[section.refParts[1]] = section;
				data[section.refParts[0]].sections[section.refParts[1]].sections= [];
			}
		});

		_.forEach(styleguide.sections, function(section){
			if(section.refParts.length == 3){
				if(_.isUndefined(data[section.refParts[0]])){
					data[section.refParts[0]] = { sections: [] };
				}
				if(_.isUndefined(data[section.refParts[0]].sections[section.refParts[1]])){
					data[section.refParts[0]].sections[section.refParts[1]] = { sections: [] };
				}
				data[section.refParts[0]].sections[section.refParts[1]].sections[section.refParts[2]] = section;
			}
		});
		...
		_.forEach(data, function(x){
			if(!_.isUndefined(x)){
				x.sections = _.compact(x.sections);
				_.forEach(x.sections, function(y){
					if(!_.isUndefined(y)){
						y.sections = _.compact(y.sections);
					}
				})
			}
		});
...
/**
* Returns a flat array of all sections within the given section
* @param  {Object}   data
*/
flattenSection = function(data){
	var flat = [_.omit(data, 'sections')];
	_.forEach(data.sections, function(section){
		flat = flat.concat(_.omit(section, 'sections'));
		if(!_.isUndefined(section.sections)){
			flat = flat.concat(_.compact(section.sections));
		}
	});
	return flat;
};
...
parseChunk = function(data, input, options) {
	...
	for (i = 0; i < l; i += 1) {
		...
		// If the markup is empty at this point we check for "template" paragraph.
		if( !currSection.markup ){
			currSection = processTemplate(paragraphs, options, currSection);
		}
		...
		if (paragraphs.length > 2) {
			...
			if (!allModifiers) {
				...
				currSection.modifiers = createModifiers(currSection.modifiers, currSection.markup, options);
			}
		}
		...
		currSection.requiresjs = requiresJS(currSection.description, options);

		// Remove prefix from description
		currSection.description = currSection.description.replace('Deprecated: ', '');
		currSection.description = currSection.description.replace('Experimental: ', '');
		currSection.description = currSection.description.replace('RequiresJS: ', '');
		...
		refParts = currSection.reference.split('.');
		for(p = 0; p < refParts.length; p++){
			refParts[p] = parseInt(refParts[p]);
		}
		currSection.refParts = refParts;
 ...
 */
createModifiers = function(lines, html, options) {
	return lines.map(function(entry) {
		...
		var modifierClass = className.replace(/\./g, ' ');

		if(html.length > 0){
			markup = html.replace(/\{\$modifiers\}/g, modifierClass);
		}
...
/**
* Look for 'Template: ' and if found read and return the contents.
* @param  {Array}  paragraphs  An array of the paragraphs in a single block
* @param  {Object}  options     The options passed on from previous functions
* @return {String}
*/
processTemplate = function(paragraphs, options, currSection){
	for(i = 0; i < paragraphs.length; i++){
		if(hasPrefix(paragraphs[i], options, 'Template')){
			var template = paragraphs[i].replace("Template:", "").replace(/^\s+|\s+$/g, '');
			currSection.template = template;
			currSection.markup = fs.readFileSync(template, 'utf8');
...
/**
* Look for 'Tags: ' and if found read and return the contents.
* @param  {Array}  paragraphs  An array of the paragraphs in a single block
* @param  {Object}  options     The options passed on from previous functions
* @return {String}
*/
processTags = function(paragraphs, options, currSection){
	for(i = 0; i < paragraphs.length; i++){
		if(hasPrefix(paragraphs[i], options, 'Tags')){
			var tags = paragraphs[i].replace("Tags:", "").replace(/^\s+|\s+$/g, '');
			currSection.tags = tags.split(',');
			paragraphs.splice(i, 1);
		}
	}
	return currSection;
...
/**
* Check if the description indicates that a section requires javascript.
* @param  {String}  description The description of that section
* @param  {Object}  options     The options passed on from previous functions
* @return {Boolean}
*/
requiresJS = function(description, options){
	return hasPrefix(description, options, 'RequiresJS');
}
